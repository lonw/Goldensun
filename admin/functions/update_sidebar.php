...
if (!empty($name)) {
  ...
  if(!empty($attachment)||!empty($link)){
    ...
    if ($uploadOk == 0) {
      $sql="UPDATE sidebar SET name='$name', link='$link', lang='$lang' where id='$id'";
    ...
    // if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES["attachment"]["tmp_name"], $target_file)) {
          $sql="UPDATE sidebar SET name='$name', link='$link', lang='$lang',attachment='$target_file' where id='$id'";
          $res = mysql_query($sql) or die (mysql_error());
            echo "The file ". basename( $_FILES["attachment"]["name"]). " has been uploaded.";
            header('location:../sidebar.php?message=success');
        } else {
