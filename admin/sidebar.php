...
if (isset($_GET['selling_id'])) {
  $id = $_GET['selling_id'];
  $sql="UPDATE links SET available='0' where id='$id'";
  $res = mysql_query($sql) or die (mysql_error());
  header('location:sidebar.php?message=sold');
}
if (isset($_GET['sold_id'])) {
  $id = $_GET['sold_id'];
  $sql="UPDATE products SET available='1' where id='$id'";
  $res = mysql_query($sql) or die (mysql_error());
  header('location:sidebar.php?message=selling');
}
?>
      	 ...
      	 <table class="table table-striped">
                  <thead>
                    ...
                    <?php
											...
											$page_query = mysql_query("SELECT COUNT(*) FROM links");
											$pages = ceil(mysql_result($page_query, 0) / $per_page);

											$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
											$start = ($page - 1) * $per_page;

											$query = mysql_query("SELECT * FROM links LIMIT $start, $per_page");
                          ...
                          <td></td>
                          <td>
                          <a href="edit-sidebar.php?id=<?php echo $data['id']; ?>" class="btn">
                          <i class="icon-edit"></i></a> <a href="functions/delete-sidebar.php?id=<?php echo $data['id']; ?>" class="btn" onclick="return confirm('To continue ?')")><i class="icon-trash"></i></a></td>
                      </tr>
											<?php
                    };
											?>
                ...
                <div class="navbar navbar-inner block-header">
                	 <a href="add-sidebar.php" class="btn"><i class="icon-plus"></i> Add</a>
                </div>
            </div>
            <!-- /block -->
        </div>
